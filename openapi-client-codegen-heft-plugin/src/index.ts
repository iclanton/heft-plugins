...
export class OpenApiCodegenPlugin
  implements IHeftPlugin<IOpenApiCodegenPluginOptions>
{
      ...
      build.hooks.preCompile.tap(PLUGIN_NAME, (preCompile) => {
        preCompile.hooks.run.tapPromise(PLUGIN_NAME, async () => {
          const logger: ScopedLogger =
            heftSession.requestScopedLogger("openapi-codegen");
          ...
          try {
            ...
            logger.emitError(
              new Error(
                `Unable to resolve source file "${options.sourceFile}": ${error}`
              )
            );
            return;
          }
          ...
          const result: IRunResult = await new Promise(
            (resolve: (result: IRunResult) => void) => {
